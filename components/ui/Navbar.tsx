        ...
        >
            ...
            >
                <div className="flex items-center justify-between gap-3 md:hidden">
                    <a href="/" className="flex items-center gap-3" aria-label="OMYTECH Kenya home">
                ...
                <div className="-mx-4 mt-2 border-t border-white/10 pt-2 md:hidden">
                    <div className="flex items-stretch justify-center gap-2 px-4">
                        {mobilePrimaryLinks.map(({ label, icon: Icon }) => {
                            const href = label === 'Services' ? '/services' : label === 'Solutions' ? '/solutions' : '/contact';
                            return (
                                <a
                                    key={label}
                                    href={href}
                                    ...
                                    style={{ color: isActive(href) ? colors.primaryLight : colors.white }}
                                >
                                    <Icon className="h-3.5 w-3.5 shrink-0 self-center" aria-hidden="true" />
                                    <span className="flex items-center leading-none">{label}</span>
                                </a>
                            );
                        })}
                    </div>
                ...
                {mobileMenuOpen && (
                    <div className="mt-2 space-y-2 border-t border-white/10 pt-3 md:hidden">
                        {mobileMoreLinks.map(({ label, icon: Icon }) => {
                            const href = label === 'Home' ? '/' : label === 'Portfolio' ? '/portfolio' : '/about';
                            return (
                                <a
                                    key={label}
                                    href={href}
                        ...
                        <a
                            ...
                            style={{ background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.accent} 100%)`, color: colors.white }}
                        >
                    ...
                    <div className="hidden items-center gap-8 md:flex">
                        {navItems.map((item) => (
                            <a
                                ...
                                style={{ color: isActive(item.href) ? colors.primaryLight : colors.white }}
                            >
                                {item.label}
                            </a>
                        ))}
