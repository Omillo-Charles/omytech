...
const projects = [
  {
    ...
    features: ['Multi-brand storefront', 'Product catalogues', 'E-commerce experience'],
  },
  {
    ...
    features: ['Portfolio showcase', 'News display', 'Media-focused experience'],
  },
  {
    ...
    features: ['Service presentation', 'Lead-focused structure', 'Responsive website'],
  },
  {
    ...
    features: ['Church information', 'Community connection', 'Mission-led content'],
  },
  {
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:gap-20 lg:px-8">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-base leading-8 text-[#536579] sm:text-lg">
              From online stores and media platforms to church websites and event systems, we build digital experiences around the way people actually work, connect, and grow.
            </p>
          </div>

          <div className="border border-[#cfe0ee] bg-white p-6 shadow-[0_18px_50px_rgba(7,26,45,0.07)] sm:p-7">
            <p className="text-xs font-semibold uppercase text-[#6b7d90]">What we build</p>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              {capabilities.map(({ label, icon: Icon }) => (
                <div key={label} className="flex items-center gap-3 border border-[#e1eaf1] bg-[#f8fbfd] p-3">
                  <Icon className="h-4 w-4" style={{ color: colors.primary }} aria-hidden="true" />
      ...
      <section className="py-16 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between gap-6">
            ...
            <span className="hidden text-sm font-semibold text-[#9aabba] sm:block">01 / {String(projects.length).padStart(2, '0')}</span>
          </div>

          <div className="mt-10 grid gap-6 sm:mt-12 lg:grid-cols-2">
            {projects.map(({ name, type, location, url, image, tone, icon: Icon, description, features }, index) => (
              <article
                key={name}
              ...
              >
                <div className={`relative min-h-[190px] overflow-hidden ${tone} ${index === 0 ? 'lg:min-h-full' : ''}`}>
                  <Image
                    src={image}
                    alt={`${name} logo`}
                    fill
                ...
                <div className="flex flex-1 flex-col pt-6 lg:pt-0">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-semibold uppercase" style={{ color: colors.primary }}>{type}</p>
                      <h3 className="mt-3 text-2xl font-bold sm:text-3xl" style={{ fontFamily: 'var(--font-glacial-indifference), sans-serif' }}>
                        {name}
                      </h3>
                    </div>
                    <Icon className="mt-1 h-5 w-5 shrink-0" style={{ color: colors.primary }} aria-hidden="true" />
                  </div>
                  <p className="mt-4 text-sm leading-7 text-[#536579]">{description}</p>
                  <div className="mt-5 flex flex-wrap gap-2">
                    {features.map((feature) => (
                      <span key={feature} className="border border-[#dce5ef] px-2.5 py-1 text-[11px] font-semibold text-[#6b7d90]">
                        {feature}
                      </span>
                    ))}
                  </div>
                  <div className="mt-7 flex flex-wrap items-center justify-between gap-3 border-t border-[#e6eef5] pt-5">
                    <span className="text-xs font-medium text-[#8a9aaa]">{location}</span>
                    <a href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-[#0b78b7] transition hover:text-[#071a2d]">
                      Visit project
                      <FiArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" aria-hidden="true" />
                    </a>
                  </div>
                </div>
              </article>
      ...
      <section className="bg-[#f2f7fb] py-16 sm:py-20">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_auto] lg:items-center lg:px-8">
          <div>
            ...
            <h2 className="mt-4 max-w-2xl text-3xl font-black leading-tight sm:text-4xl" style={{ fontFamily: 'var(--font-glacial-indifference), sans-serif' }}>
              Have something worth putting online?
            </h2>
            <p className="mt-4 max-w-2xl text-base leading-8 text-[#536579]">
              Tell us what you are building, and we will help you find the right digital shape for it.
            </p>
          </div>
